...
export const getDocument = async (req, res, next) => {
  try {
    ...
    if (document.generatedBy._id.toString() !== req.user.id && req.user.role !== 'admin') {
...
export const getDocumentStats = async (req, res, next) => {
  try {
    ...
    const totalDocuments = await Document.countDocuments({ generatedBy: userId });
    const recentDocuments = await Document.find({ generatedBy: userId })
      .sort({ createdAt: -1 })
      .limit(5)
